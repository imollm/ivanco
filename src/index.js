...
if (stagedFiles.length === 0 && changedFiles.length > 0) {
    ...
    if (isCancel(files))
        handleExit({ message: 'You didn\'t selected any file to be commited.'})
...
if (isCancel(commitType))
    handleExit({ message: 'You have not selected commit type.' })
...
if (isCancel(commitMsg))
    handleExit({ message: 'You have not written the commit message.' })
...
if (!shouldContinue)
    handleExit({ message: 'The commit was not created!' })
